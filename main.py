    ...
    try:
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            mp4_formats = [
                ...
                if (
                    (str(url).startswith("https://www.tiktok.com/@") and fmt.get('cookies') is None and fmt.get("width") >= 720) or
                    (str(url).startswith("https://www.youtube.com") and fmt.get('ext') == 'mp4' and fmt.get('protocol') == 'https' and fmt.get('width') >= 720) or
                    (str(url).startswith("https://www.instagram.com") and fmt.get('width') is not None and fmt.get("width") >= 540 and not str(fmt.get("format")).startswith("dash-"))
                )
            ]
